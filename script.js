...
checkButton.addEventListener("click", () => {
  ...
  if (inputValue == randomNum) {
    ...
    [guess.textContent, input.disabled] = ["Tebrikler", true];
    [checkButton.textContent, guess.style.color] = ["Tekrar", "#333"];
    //Check if input value is > random number and within 1-99 range.
  } else if (inputValue > randomNum && inputValue < 100) {
    // Update the guess text and remaining chances
    [guess.textContent, remainChances.textContent] = ["Tahminin yüksek", chance];
    guess.style.color = "#333";
    //Check if input value is < random number and within 1-99 range.
  } else if (inputValue < randomNum && inputValue > 0) {
    // Update the guessed number text and remaining chances
    [guess.textContent, remainChances.textContent] = ["Tahminin düşük", chance];
    guess.style.color = "#333";
  ...
  if (chance == 0) {
    ...
    [checkButton.textContent, input.disabled, inputValue] = ["Tekrar", true, ""];
    [guess.textContent, guess.style.color] = ["Oyunu kaybettin", "#DE0611"];
  }
  if (chance < 0) {
